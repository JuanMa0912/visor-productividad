        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">
          <tr>
            <td colspan="2" style="padding:12px 14px;border-radius:12px;background:#fff1f2;border:1px solid #fecdd3;">
              <div style="font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:#be123c;">Total D+0+S</div>
          ...
          <tr>
            ...
            <td style="padding:6px 0;text-align:right;font-weight:700;color:#0f172a;">${formatCount(digest.demandaD.itemCount)}</td>
          </tr>
          <tr>
            <td style="padding:6px 0;color:#475569;">Total inventario</td>
            <td style="padding:6px 0;text-align:right;font-weight:700;color:#0f172a;">${formatPriceWithoutSixZeros(digest.demandaD.totalInventario)}</td>
          </tr>
          <tr>
            <td style="padding:6px 0;color:#475569;">Días de inventario</td>
            <td style="padding:6px 0;text-align:right;font-weight:700;color:#0f172a;">${formatDiasInventario(digest.demandaD.diasInventario)}</td>
          </tr>
          ${renderEstadoBlock("0 · Cero rotación", "#475569", digest.ceroRotacion)}
          ${renderEstadoBlock("S · Restock / en stock", "#0e7490", digest.restockS)}
        </table>
      </td>
    </tr>
    <tr>
      <td style="padding:16px 24px 24px;border-top:1px solid #f1f5f9;font-size:12px;line-height:1.5;color:#94a3b8;">
        Resumen automático del visor de productividad. Los porcentajes de surtido corresponden al estado S.inventario registrado en rotación.
      </td>
    </tr>
  </table>
</body>
...
export const buildRotacionCriticalDigestText = (
  ...
  const rangeLabel = formatRangeLabel(digest.dateRange);
  const estadoLines = (label: string, b: SurtidoEstadoBreakdown) =>
    [
  ...
  return [
    ...
    "",
    `D · DEMANDA`,
    `  Ítems: ${formatCount(digest.demandaD.itemCount)}`,
    `  Total inventario: ${formatPriceWithoutSixZeros(digest.demandaD.totalInventario)}`,
    `  Días de inventario: ${formatDiasInventario(digest.demandaD.diasInventario)}`,
    "",
    estadoLines("0 · CERO ROTACIÓN", digest.ceroRotacion),
    "",
    estadoLines("S · RESTOCK", digest.restockS),
  ].join("\n");
};
