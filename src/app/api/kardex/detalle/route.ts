...
import { getKardexDetalle } from "@/features/kardex/repo";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const rows = await getKardexDetalle(filters);
    ...
    console.error("[kardex][detalle] error", {
      error: error instanceof Error ? error.message : String(error),
    });
    return NextResponse.json(
      { error: "Error interno consultando kardex detalle." },
