...
import { getResumenPorItem } from "@/features/kardex/repo";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const rows = await getResumenPorItem(filters);
    ...
    console.error("[kardex][resumen-item] error", {
      error: error instanceof Error ? error.message : String(error),
    });
    return NextResponse.json(
      { error: "Error interno consultando kardex por item." },
