...
type RotationApiResponse = {
  ...
  filters: {
    ...
    /** Sublíneas N2 cuando se consulta con lineaN1Scope. */
    lineasN2?: string[];
    lineasN2Nombres?: Record<string, string>;
    /** Mapa empresa::sedeId::item -> codigo N2 (catalogo con lineaN1Scope). */
...
/** Vista restock: columna Auditar entre S.inventario y Descripcion. */
const ROTACION_RESTOCK_TABLE_COL_WIDTHS = [
  "4%",
  "7%",
  "4%",
  "9%",
...
const ROTACION_FLOATING_HEADER_COLUMNS_RESTOCK = [
  { label: "#", align: "right" as const },
  { label: "Item", align: "left" as const, field: "item" as const },
  {
    label: "Cat.",
  ...
  { label: "Auditar", align: "center" as const },
  {
    label: "Descripcion",
    align: "left" as const,
    field: "descripcion" as const,
...
const mergeRotationLineaN2NombreMaps = (
  base: Record<string, string> | undefined,
  extra: Record<string, string> | undefined,
): Record<string, string> => {
  const out = { ...(base ?? {}) };
...
const normalizeLineaN2CodeForFilter = (
  raw: string | null | undefined,
): string => {
  const t = String(raw ?? "").trim();
...
const compareLineaN2FilterCodes = (a: string, b: string): number => {
  if (a === b) return 0;
  if (a === "__sin_n2__") return -1;
  if (b === "__sin_n2__") return 1;
  const aNum = /^\d+$/.test(a) ? Number.parseInt(a, 10) : Number.NaN;
  const bNum = /^\d+$/.test(b) ? Number.parseInt(b, 10) : Number.NaN;
  const aIsNum = Number.isFinite(aNum);
  const bIsNum = Number.isFinite(bNum);
  ...
  rows
    .map((row) => {
      return {
        ...
        lineaN2Codigo: row.lineaN2Codigo ?? null,
        sublinea: row.sublinea ?? null,
      };
    })
...
/** Lineas N2 para filtro en cliente: null = sin filtro (seleccion completa). */
const buildLineasN2ClientFilterValues = (
  uiOptions: string[],
  selectedValues: string[],
): string[] | null => {
  const normalizedUi = uiOptions.map(normalizeLineaN2CodeForFilter);
  const normalizedSelected = selectedValues.map(normalizeLineaN2CodeForFilter);
  if (normalizedUi.length === 0) {
    return normalizedSelected.length > 0 ? normalizedSelected : null;
  }
  if (normalizedSelected.length === 0) return null;
  const isFull =
    normalizedSelected.length === normalizedUi.length &&
    normalizedUi.every((value) => normalizedSelected.includes(value));
  if (isFull) return null;
  return normalizedSelected;
};

/** Lineas N2 a enviar en query: null = sin filtro (todo el catalogo o vacio). */
const buildLineasN2QueryValues = (
  catalog: string[],
  selectedValues: string[],
): string[] | null => {
  const normalizedCatalog = catalog.map(normalizeLineaN2CodeForFilter);
  const normalizedSelected = selectedValues.map(normalizeLineaN2CodeForFilter);
  if (normalizedCatalog.length === 0) {
    return normalizedSelected.length > 0 ? normalizedSelected : null;
  }
  const catalogSet = new Set(normalizedCatalog);
  const valid = normalizedSelected.filter((value) => catalogSet.has(value));
  const isFull =
    valid.length === normalizedCatalog.length &&
    normalizedCatalog.every((value) => valid.includes(value));
  if (valid.length === 0 || isFull) return null;
  return valid;
};

/** Clave estable para cruzar item con su sublinea N2. */
...
const resolveRowLineaN2FilterCode = (
  row: RotationRow,
  nombresByCode: Record<string, string> = {},
  itemLineaN2ByKey: Record<string, string> = {},
): string => {
  const direct = normalizeLineaN2CodeForFilter(row.lineaN2Codigo);
  if (direct !== "__sin_n2__") return direct;

  const fromIndex = itemLineaN2ByKey[
    buildRotationItemLineaN2Key(row.empresa, row.sedeId, row.item)
  ];
  if (fromIndex) return normalizeLineaN2CodeForFilter(fromIndex);

  const sub = row.sublinea?.trim();
  if (sub && sub.toLowerCase() !== "sin sublinea") {
    const subFold = foldForProductSearch(sub);
    for (const [code, name] of Object.entries(nombresByCode)) {
      if (code === "__sin_n2__" || !name) continue;
      const nameFold = foldForProductSearch(name);
      if (nameFold === subFold || nameFold.includes(subFold) || subFold.includes(nameFold)) {
        return normalizeLineaN2CodeForFilter(code);
      }
    }
  }
...
const rowMatchesLineaN2Filter = (
  row: RotationRow,
  selectedCodes: string[],
  nombresByCode: Record<string, string>,
): boolean => {
  const normalizedSelected = selectedCodes.map(normalizeLineaN2CodeForFilter);
  const selectedSet = new Set(normalizedSelected);
  const resolved = resolveRowLineaN2FilterCode(row, nombresByCode);
  if (selectedSet.has(resolved)) return true;

  const sub = row.sublinea?.trim();
  if (!sub || sub.toLowerCase() === "sin sublinea") return false;
  const subFold = foldForProductSearch(sub);

  for (const code of normalizedSelected) {
    const name = nombresByCode[code];
    if (!name) continue;
    const nameFold = foldForProductSearch(name);
    if (
      nameFold === subFold ||
      nameFold.includes(subFold) ||
      subFold.includes(nameFold)
    ) {
      return true;
    }
  }
...
/** Filtra filas ya cargadas (misma logica que lineasN1/categoria en /api/rotacion). */
const filterRotationRowsByLineaAndCategoria = (
  rows: RotationRow[],
  catalogLineasN1: string[],
  selectedLineasN1: string[],
  catalogCategorias: RotationCategoriaFilterOption[],
  selectedCategoriaKeys: string[],
  catalogLineasN2: string[] = [],
  selectedLineasN2: string[] = [],
  lineasN2Nombres: Record<string, string> = {},
  itemLineaN2ByKey: Record<string, string> = {},
): RotationRow[] => {
  const lineasFilter = buildLineasN1QueryValues(
    catalogLineasN1,
    selectedLineasN1,
  );
  const normalizedN2Catalog = catalogLineasN2.map(normalizeLineaN2CodeForFilter);
  const normalizedN2Selected = selectedLineasN2.map(normalizeLineaN2CodeForFilter);
  const lineasN2Filter = buildLineasN1QueryValues(
    normalizedN2Catalog,
    normalizedN2Selected,
  );
  const catsFilter = buildCategoriaQueryKeys(
    catalogCategorias,
    selectedCategoriaKeys,
  );
  let result = rows;
  if (lineasFilter) {
    const lineSet = new Set(lineasFilter);
    result = result.filter((row) =>
      lineSet.has(normalizeLineaN1CodeForFilter(row.lineaN1Codigo)),
    );
  }
  if (lineasN2Filter) {
    const lineN2Set = new Set(lineasN2Filter);
    result = result.filter((row) =>
      lineN2Set.has(
        resolveRowLineaN2FilterCode(row, lineasN2Nombres, itemLineaN2ByKey),
      ),
    );
  }
  if (catsFilter) {
    const catSet = new Set(catsFilter);
    result = result.filter((row) =>
      catSet.has(normalizeRotationCategoriaKey(row.categoria)),
    );
  }
  return result;
};
