...
const filterRotationRowsByLineaAndCategoria = (
  ...
  const lineasN2Filter = buildLineasN2ClientFilterValues(
    catalogLineasN2,
    selectedLineasN2,
  );
  ...
  if (lineasN2Filter) {
    result = result.filter((row) =>
      rowMatchesLineaN2Filter(row, lineasN2Filter, lineasN2Nombres),
    );
  }
  if (catsFilter) {
