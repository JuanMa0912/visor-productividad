...
| `rotacion` | rotacion e inventario con baja salida; capitulos A-B-C, criticos (D+0+S) y sobrestock (32+ / 50+ dias de inventario); filtro CAT (multi); restock muestra conteo S.inventario (sin verificar / seguimiento / surtido); tab Grafico: barras apiladas D+0+S (mismo corte del correo diario; default todas las sedes / manufactura), agrupacion sede/linea/sublinea/item y metrica cantidad/inventario/unidades |
