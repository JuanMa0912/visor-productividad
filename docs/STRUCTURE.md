...
| `rotacion/tendencia` | POST venta diaria `$` de una sede (`empresa`, `sedeId`, `start`, `end`, `items` opcional); modal Tendencia en tabla Rotación. Fechas desde el **1 de junio** del año. Recorta por A/B/C/D, 0, restock o cero rotación; ignora días de inventario. Sin recorte de ítems consulta toda la sede |
