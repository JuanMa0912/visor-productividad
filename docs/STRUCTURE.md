...
| `rotacion/tendencia` | POST serie diaria de una sede (`empresa`, `sedeId`, `start`, `end`, `items` opcional): `sales`, `units` (inventario und) y `inventoryValue`; modal Tendencia con filtros Ventas / Unidades / Valor inventario. En cero rotación y restock arranca en inventario. Fechas desde el **1 de junio** del año. Recorta por A/B/C/D, 0, restock o cero rotación; ignora días de inventario |
