...
| `proveedores/visitas` | subtablero `proveedores`: pestaña **Asistencia** (entrada/salida en `qr_*`, listado/filtros/CSV + métricas por `entrada_at` en calendario `America/Bogota`) y pestaña **QR** (links/PNG; solo `proveedores_qr` o admin); `mode=meta` con links QR; ancla inicial = último día con venta |
