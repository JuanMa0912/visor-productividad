...
| `costos` | tablero `/costos` (subtablero opt-in `precios-proveedor`; `/exp/precios-proveedor` redirige): heatmap ítem×sede; **costo de entrada = inventario ET/EF** (`cmmovimiento_inventario` en 217 → `orden_compra_linea` / `rotacion_salidas_dia`; si no hay ET/EF ese día, FR/OC); Mercatodo: ET tránsito + EF; precio venta no se toca; doble clic: $/kg, kilos y margen vendido; al entrar consulta **todas las sedes** del catálogo (empresa/sede vacíos = todas); filtros multi-select: empresa, sede, línea, sublínea, ítem y proveedor; proveedor sin prefijo de empresa (Bogotá/Mercamio/Mercatodo); al expandir, el mismo nombre se agrupa en una fila y suma el costo; máx. 31 días |
