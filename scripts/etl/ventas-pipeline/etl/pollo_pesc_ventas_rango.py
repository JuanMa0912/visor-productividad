...
# =========================
CATEGORIA = "4"
LINEAS = ["03", "04"]  # pollo + pescado (items.id_linea1)

# =========================
# SQL ORIGEN (PARAMETRIZADO)
# - Usamos ANY(%s) para lista de líneas
# - OJO: 'Z%%' en SQL para LIKE con % dentro de string parametrizado
# =========================
SQL_ORIGEN = """
SELECT
...
CREATE TABLE IF NOT EXISTS {DEST_DB["schema"]}.{DEST_DB["table"]} (
    ...
    CONSTRAINT pk_ventas_pollo_pesc PRIMARY KEY (
        empresa_bd, centro_operacion, sede, caja, fecha_dcto, id_tipdoc_fc, documento_fc, id_vend_cc, categoria, linea
    )
);
"""
...
ON CONFLICT (empresa_bd, centro_operacion, sede, caja, fecha_dcto, id_tipdoc_fc, documento_fc, id_vend_cc, categoria, linea)
DO UPDATE SET
...
def fetch_origen(cfg: dict, fecha_ini: str, fecha_fin: str) -> pd.DataFrame:
    ...
    try:
        # 4 placeholders -> 4 params
        params = (cfg["empresa"], fecha_ini, fecha_fin, CATEGORIA, LINEAS)
...
def load_destino(df: pd.DataFrame) -> int:
    ...
    # Normalizar columnas texto (por si vienen char/numeric)
    text_cols = [
        "empresa_bd", "centro_operacion", "sede", "caja", "fecha_dcto", "id_tipdoc_fc", "documento_fc",
        "id_vend_cc", "categoria", "linea"
    ]
...
def main():
    ...
    print(f"ETL ventas_pollo_pesc | rango={fecha_ini}..{fecha_fin} | categoria={CATEGORIA} | lineas={LINEAS}")
