...
# OJO: 'Z%%' para LIKE dentro de SQL con placeholders %s
# =========================
SQL_ORIGEN = """
SELECT
...
CREATE TABLE IF NOT EXISTS {DEST_DB["schema"]}.{DEST_DB["table"]} (
    ...
    CONSTRAINT pk_ventas_cat3_l01 PRIMARY KEY (
...
def fetch_origen(cfg: dict, fecha_ini: str, fecha_fin: str) -> pd.DataFrame:
    ...
    try:
        params = (cfg["empresa"], fecha_ini, fecha_fin, CATEGORIA, LINEA1)  # 4 placeholders -> 4 params
...
def main():
    ...
    print(f"ETL {DEST_DB['table']} | rango={fecha_ini}..{fecha_fin} | categoria={CATEGORIA} | linea1={LINEA1}")
