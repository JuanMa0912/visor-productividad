...
- **Tablas (allowlist):** `ventas_cajas`, `ventas_fruver`, `ventas_carnes`,
  ...
  `proveedor_item`, `proveedor_tercero`, `orden_compra`, `orden_compra_linea`, `margen_final`.
- **`orden_compra` / `orden_compra_linea`:** **no** entran en el diario 07:50 ni
  en el reconcile. La carga POS→232 (dias nuevos + abiertas) y el upsert a GCP
  los hace `visor-etl-orden-compra` (08:00) con `--only orden_compra --only orden_compra_linea`
  (tablas locales completas, sin borrar GCP). Si el local esta vacio, no toca GCP.
...
- Al terminar refresca en GCP (inline, sin scripts externos): la **matview de rotacion**
  ...
  Complemento en **app-server**: `visor-refresh-variacion.timer` (08:15) reconstruye
...
- `visor-etl-sync.{service,timer}` -> **todos los dias 07:50**, sube ayer (todas las tablas **excepto** `orden_compra` / `orden_compra_linea`).
- `visor-etl-reconcile.{service,timer}` -> **domingos 16:00**, re-sube los ultimos 7 dias (tampoco toca OC).
- `visor-etl-orden-compra.{service,timer}` -> **todos los dias 08:00**, POS→232 incremental + `$SYNC --only orden_compra --only orden_compra_linea`.
...
- El reconcile corre con **`--replace`**: reemplaza en GCP las fechas presentes en el
  ...
  `orden_compra_linea` (timer `visor-etl-orden-compra` a las 08:00, siempre con `--only`).
